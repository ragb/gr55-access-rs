...
- [ ] Tempo / BPM encoding.
- [x] **Current Patch encoding.** **Resolved (HIGH):** 2 wire bytes at `[01, 00, 00, 00]` and `[01, 00, 00, 01]`, 14-bit MSB-first (`linear_index = byte_hi * 128 + byte_lo`), decoded via `PatchSlot::from_linear_index`. The codegen's apparent 19 leaf parameters at `[01, 00, 00, 0x00..0x12]` are an artifact of `midi.xml`'s structure: under `<PARAM name="Current patch">` there are 19 `<DATA value="00".."12">` siblings, each enumerating the patch names visible *for that high-byte value* (e.g. `<DATA value="00">` starts at `User 01:1`, `<DATA value="01">` starts at `User 43:3` = index 128). The XML's `value` attr on those DATA elements is a high-byte discriminator, not an address offset; only two bytes are written on the wire. Verified by decoding `system.syx` frame 1 bytes `[0x00, 0x39]` → linear index 57 → User 20:1 and round-tripping every USER slot.
